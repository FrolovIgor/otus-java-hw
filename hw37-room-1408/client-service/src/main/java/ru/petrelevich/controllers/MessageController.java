...
import ru.petrelevich.contants.Constants;
...
public class MessageController {
    ...
    public void getMessage(@DestinationVariable String roomId, Message message) {
        ...
        if (roomId.equals(String.valueOf(Constants.SECRET_ROOM_ID))) {
            logger.error("It's not allowed to send messages from Secret Room!");
            return;
        }
        ...
        template.convertAndSend(String.format("%s%s", TOPIC_TEMPLATE, Constants.SECRET_ROOM_ID), msg);
    }
    ...
    public void handleSessionSubscribeEvent(SessionSubscribeEvent event) {
         ...
         */
        if (Constants.SECRET_ROOM_ID == roomId) {
    ...
    private String getUserPath(String simpDestination) {
            var idxRoom = simpDestination.lastIndexOf(TOPIC_TEMPLATE);
            return simpDestination.substring(0,idxRoom);
    ...
    private Flux<Message> getMessagesByRoomId(long roomId) {
        return datastoreClient
                ...
                .exchangeToFlux(this::clientResponseToFlux);
    }
    ...
    private Flux<Message> clientResponseToFlux(ClientResponse response) {
